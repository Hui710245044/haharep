...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            //是否启用
            changeSelect(index){
                this.searchData.status = this.userStatus[index].status;
                this.init()
            },
            //选择平台
            change_channel(index){
                this.searchData.platform = this.channelList[index].value;
                this.init()
            },
            //获取平台
            get_channel(){
...
//                    });
            ...
            add_update(val,id,action){
                console.log('val',val);
                console.log('111', this.searchData.trigger_rule);
            	...
            	if(!!find){
            	    ...
            	    this.$set(data,'create_time',new Date().getTime()/1000);
