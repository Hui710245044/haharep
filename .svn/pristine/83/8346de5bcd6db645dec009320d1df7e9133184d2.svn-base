<template>
    <div class="bundling-sales">
        <page-dialog title="捆绑/打包销售" v-model="dialog" size="large" :close-on-click-modal="false" >
            <div class="reminder">
            ...
            <el-button class="inline mt-sm" type="primary" size="mini" @click.native="add_new_bundling">添加捆绑产品</el-button>
            <!--添加产品  弹出页面-->
            ...
            >
                <el-table-column label="本地SKU" prop="local_sku"></el-table-column>
                <el-table-column label="捆绑销售数量" inline-template>
                    <el-input-number v-model="row.quantity"
                                     :min="1"
                                     @input="input(row)"
                                     :controls = "false"
                                     class="inline width-sm"></el-input-number>
                    <!--<input class="inline width-sm el-input__inner" v-model="row.quantity" type="number" min="1" onkeyup="this.value=this.value.replace(/\D/g,'')"onafterpaste="this.value=this.value.replace(/\D/g,'')" >-->
                </el-table-column>
