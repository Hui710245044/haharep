<template>
    <el-row class="aliexpress-has-published">
        ...
        <el-row class="mb-sm mt-sm ml-sm">
            <permission tag="request-button" :mintime="200" req-key="url_aliexpress_batch_del1" @click="batchdel" :route="apis.url_aliexpress_batch_del">批量删除</permission>
        </el-row>
        <!--table数据-->
        <ui-table
                ref="table"
...
<script>
    import {api_aliexpress_list_fail,api_aliexpress_account,api_aliexpress_product_status,api_aliexpress_expire_search,api_aliexpress_batch_del,api_aliexpress_sale_list,url_aliexpress_edit_product,url_aliexpress_batch_del} from "../../../../api/publish-smt"
    import {operateList} from './action';//------操作按钮引入
    import {get_path} from '../../ebay/kandeng-list/add-edit-listing/get-path';
    export default{
        permission: {
            url_aliexpress_edit_product,url_aliexpress_batch_del
        },
        data(){
            return {
                pickerstart:{
