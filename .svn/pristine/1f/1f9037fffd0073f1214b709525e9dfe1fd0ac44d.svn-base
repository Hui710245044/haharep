        ...
        <div class="left-box">
            <search-card class="card" @search="search"
                ...
                <div v-if="form.status==='0_256'" class="mb-mini">
                    ...
                    <label class="line ml-lg">筛选手工订单：</label>
                    <el-select class="inline width-lg" v-sf.fil_handle v-model="form.fil_handle">
                        <el-option
                            :key="item.value"
            ...
            <div class="card left-main">
                <div class="mt-sm mb-sm">
                    ...
                    <permission
                        class="ml-sm"
                        tag="request-button"
                        :route="apis.url_audited"
                        v-if="form.status === '0_256'"
                        req-key="apis_url_audited"
                        @click="mark_check">标记为已审核
                    </permission>
                    <!--过期-->
                    <permission
                        class="ml-sm"
...
<script>
    ...
    import {
        ...
        api_orders_export_title, api_info
    } from '../../../api/order-local';
...
//            需人工审核 固定数字 3；
        ...
        methods: {
            ...
            init_params() {
                ...
                let curString = this.snText.replace(new RegExp(' ', 'gm'), '\n');
                ...
                if (this.form.snType && this.snText) {
                    params.snType = this.form.snType;
                }
                return params
            },
            init_status_btn() {
                let cur = this.unusualList;
                let cur2 = this.buttons;
                this.unusualList = [];
                this.buttons = [];
                this.$nextTick(() => {
                    this.unusualList = cur;
                    this.buttons = cur2;
                })
            },
            order_export(params,head) {
                ...
                // this.$http(api_orders_audit_export, params).then(res => {
                //     let url = config.apiHost + 'downloadFile/downExportFile';
                //     let params = {
                //         file_code: res.file_code,
                //         page: this.tables.page,
...
//            标记为已审核
            mark_check() {
                if (this.selectedItems.length <= 0) {
                    this.$reqKey('apis_url_audited', false);
                    this.$message({
                        type: 'warning',
                        message: '所要标记的订单'
                    });
