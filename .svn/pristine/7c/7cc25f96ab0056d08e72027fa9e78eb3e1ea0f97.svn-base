<template>
    <el-row class="p-wish-order-table">
        ...
        <ui-table v-loading="loading" :first-loading="firstLoading" element-loading-text="玩命加载中..." :has-data="dataTable.length>0" v-model="checkAll"
            ...
            <template v-for="(data, index) in dataTable">
                <tr class="line" :class="[data.show ? 'active' : '',data.clickData?'active-color':'']"
                    ...
                    <td>
                        <times v-if="data.pay_time" :time="data.pay_time" :width="88"></times>
                        <label v-else>未付款</label>
                    </td>
                    <td>
                        <ui-tip :content="data.transaction_id" :width="88"></ui-tip>
                    </td>
                    <td>
        ...
        <import-excel v-model="showImport" @files-update="files_update" :export-api="export_transfer_template" :import-api="import_transfer" :select-list="selectedList"></import-excel>
    </el-row>
</template>
<style lang="stylus">
    .p-wish-order-table {
