<template>
    <ui-card v-if="isAdd === false">
        ...
        <div>
            <el-table :data="copyData" border class="scroll-bar" v-resize="{height: 40}" highlight-current-row>
            ...
            <look-edit v-model="newVisable" :editable="editable" :title="title" :express-data="expressData"
                       :baseData="baseData" @files-success="files_success" :effData="effData"></look-edit>
        </div>
...
<script>
    ...
    import {api_base, api_express, api_effective, api_ali_address, logistics_enabled} from "../../../api/setLogistics"
    ...
    export default {
        ...
        created() {
            this.copyData = window.clone(this.lists);
        },
        methods: {
            files_success(id) {
            ...
            search() {
                ...
                if (key === '') {
                    ...
                    this.copyData = this.lists.filter(res => {
        ...
        watch: {
            ...
            lists(val) {
                this.searchText = '';
                this.copyData = window.clone(val);
            }
        },
        props: {
            lists: {
