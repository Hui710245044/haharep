<template>
    <div class="c-published-draft-list">
        <el-table
            ...
            <el-table-column label="主图" inline-template width="90px">
            ...
            <el-table-column label="账号" prop="codes"></el-table-column>
    ...
    export default {
        ...
        data() {
            return {
                ...
                operateList:[
                    {label:"编辑",value:"1",apis:url_edit_amazon_draft,action:function(row){
                        let params = {
                            spu: row.spu,
                            id:row.id,
                            curPage:'draft',//未刊登
                    ...
                    {label:"删除",value:"2",apis:url_amazon_draft_delete,action:function(row){
        ...
        methods:{
            ...
            delete_listing(row){
                this.$confirm(`此操作将删除SPU为${row.spu}的草稿数据，确认此操作吗？`,"提示",{
            ...
            async get_attribute(curRow){
                ...
                await this.get_product_attribute(productType,curRow.site).then(id=>{
                    ...
                    * */
                    ...
                    if(!id)return this.$message({type:"warning",message:`【${curRow.code}】账号，暂无默认的产品模板，请添加！`});
